...
// ---------------------------------------------------------------------------
// Project-level comparative analysis types and endpoints
// ---------------------------------------------------------------------------

export interface ComparativeJob {
  id: string;
  project_id: string;
...
export interface PangenomeResult {
  id: string;
  project_id: string;
  core_genes: number;
  soft_core_genes: number;
  shell_genes: number;
  cloud_genes: number;
  total_genes: number;
}

export interface SNPTreeResult {
  id: string;
  project_id: string;
  core_snp_count: number;
  core_genome_size: number;
  newick_path: string;
  newick_recomb_filtered_path: string;
}

export interface MashtreeResultData {
  id: string;
  project_id: string;
  newick_path: string;
}

export async function startPangenome(projectId: string, sampleIds?: string[]): Promise<{ job_id: string }> {
  return fetchAPI(`/api/projects/${projectId}/comparative/pangenome`, {
    method: 'POST',
    body: JSON.stringify(sampleIds ? { sample_ids: sampleIds } : {}),
  });
}

export async function startSNPTree(projectId: string, options?: { reference_sample_id?: string; run_gubbins?: boolean; sample_ids?: string[] }): Promise<{ job_id: string }> {
  return fetchAPI(`/api/projects/${projectId}/comparative/snp-tree`, {
    method: 'POST',
    body: JSON.stringify(options || {}),
  });
}

export async function startMashtree(projectId: string, sampleIds?: string[]): Promise<{ job_id: string }> {
  return fetchAPI(`/api/projects/${projectId}/comparative/mashtree`, {
    method: 'POST',
    body: JSON.stringify(sampleIds ? { sample_ids: sampleIds } : {}),
  });
}

export async function getComparativeJobs(projectId: string): Promise<ComparativeJob[]> {
  return fetchAPI<ComparativeJob[]>(`/api/projects/${projectId}/comparative/jobs`);
}

export async function getLatestPangenome(projectId: string): Promise<PangenomeResult> {
  return fetchAPI<PangenomeResult>(`/api/projects/${projectId}/comparative/pangenome/latest`);
}

export async function getLatestSNPTree(projectId: string): Promise<SNPTreeResult> {
  return fetchAPI<SNPTreeResult>(`/api/projects/${projectId}/comparative/snp-tree/latest`);
}

export async function getLatestMashtree(projectId: string): Promise<MashtreeResultData> {
  return fetchAPI<MashtreeResultData>(`/api/projects/${projectId}/comparative/mashtree/latest`);
}
