    ...
    <div className="space-y-6">
      ...
      {data?.mobile_args && data.mobile_args.genes.length > 0 && (() => {
        ...
        const downloadMobileArgs = () => {
          const headers = ['Gene', 'Drug Class', 'Associated MGE', 'Distance (bp)', 'Strain Count', ...ma.sample_names];
          const rows = ma.genes.map((g) => [
            g.gene, g.drug_class, g.mge_names.join('; '), g.min_distance, g.strain_count,
